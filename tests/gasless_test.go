...
import (
	...
	testingDeployerContracts "github.com/kaiachain/kaia/contracts/contracts/testing/deployer"
...
func TestGasless(t *testing.T) {
	...
	// create2
	_, _, _, deployerContract := deployCreate2(t, node, owner, owner.Addr)

	// deploy contracts
	_, testTokenAddr, _, testTokenContract := deployTestToken(t, node, owner, deployerContract, owner.Addr)
	_, wkaiaAddr, _, wkaiaContract := deployWKAIA(t, node, owner, deployerContract)
	_, factoryAddr, _, factoryContract := deployUniswapV2Factory(t, node, owner, deployerContract, owner.Addr)
	_, routerAddr, _, routerContract := deployUniswapV2Router02(t, node, owner, deployerContract, factoryAddr, wkaiaAddr)
	_, gsrAddr, _, gsrContract := deployGaslessSwapRouter(t, node, owner, deployerContract, wkaiaAddr)

	t.Log(common.Bytes2Hex(node.TxPool().GetCurrentState().GetCode(routerAddr)))
...
func deployCreate2(t *testing.T, node *cn.CN, owner *TestAccountType, initialHolder common.Address,
) (uint64, common.Address, *types.Transaction, *testingDeployerContracts.Factory) {
	var (
	...
	// Deploy contract
	addr, tx, contract, err := testingDeployerContracts.DeployFactory(optsOwner, transactor)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Logf("Deployer deployed at block=%2d, addr=%s", num, addr.Hex())

	owner.Nonce++
	return num, addr, tx, contract
}

func deployTestToken(t *testing.T, node *cn.CN, owner *TestAccountType, deployerContract *testingDeployerContracts.Factory, initialHolder common.Address,
	...
	// Deploy contract
	code := common.Hex2Bytes(testingGaslessContracts.TestTokenBin)
	parsed, err := testingGaslessContracts.TestTokenMetaData.GetAbi()
	if err != nil {
		t.Fatal(err)
	}
	input, err := parsed.Pack("", initialHolder)
	if err != nil {
		t.Fatal(err)
	}
	data := append(code, input...)

	codeHash := crypto.Keccak256(data)
	var bytes32 [32]byte
	addr := crypto.CreateAddress2(owner.Addr, [32]byte(deploySalt.FillBytes(bytes32[:])), codeHash)
	tx, err := deployerContract.Deploy(optsOwner, data, deploySalt)
	if err != nil {
		t.Fatal(err)
	}
	...
	contract, err := testingGaslessContracts.NewTestToken(addr, transactor)
	if err != nil {
		t.Fatal(err)
	}
...
func deployWKAIA(t *testing.T, node *cn.CN, owner *TestAccountType, deployerContract *testingDeployerContracts.Factory,
	...
	// Deploy contract
	code := common.Hex2Bytes(testingContracts.WKAIABin)
	parsed, err := testingContracts.WKAIAMetaData.GetAbi()
	if err != nil {
		t.Fatal(err)
	}
	...
	contract, err := testingContracts.NewWKAIA(addr, transactor)
	if err != nil {
		t.Fatal(err)
	}
...
func deployUniswapV2Factory(t *testing.T, node *cn.CN, owner *TestAccountType, deployerContract *testingDeployerContracts.Factory, feeToSetter common.Address,
	...
	// Deploy contract
	code := common.Hex2Bytes(uniswapFactoryContracts.UniswapV2FactoryBin)
	parsed, err := uniswapFactoryContracts.UniswapV2FactoryMetaData.GetAbi()
	if err != nil {
		t.Fatal(err)
	}
	input, err := parsed.Pack("", feeToSetter)
	if err != nil {
		t.Fatal(err)
	}
	data := append(code, input...)
	...
	contract, err := uniswapFactoryContracts.NewUniswapV2Factory(addr, transactor)
	if err != nil {
		t.Fatal(err)
	}
...
func deployUniswapV2Router02(t *testing.T, node *cn.CN, owner *TestAccountType, deployerContract *testingDeployerContracts.Factory, factory, wkaia common.Address,
	...
	// Deploy contract
	code := common.Hex2Bytes(uniswapRouterContracts.UniswapV2Router02Bin)
	parsed, err := uniswapRouterContracts.UniswapV2Router02MetaData.GetAbi()
	if err != nil {
		t.Fatal(err)
	}
	input, err := parsed.Pack("", factory, wkaia)
	if err != nil {
		t.Fatal(err)
	}
	data := append(code, input...)
	...
	contract, err := uniswapRouterContracts.NewUniswapV2Router02(addr, transactor)
	if err != nil {
		t.Fatal(err)
	}
...
func deployGaslessSwapRouter(t *testing.T, node *cn.CN, owner *TestAccountType, deployerContract *testingDeployerContracts.Factory, wkaiaAddr common.Address,
	...
	// Deploy contract
	code := common.Hex2Bytes(gaslessContract.GaslessSwapRouterBin)
	parsed, err := gaslessContract.GaslessSwapRouterMetaData.GetAbi()
	if err != nil {
		t.Fatal(err)
	}
	input, err := parsed.Pack("", wkaiaAddr)
	if err != nil {
		t.Fatal(err)
	}
	data := append(code, input...)
	...
	contract, err := gaslessContract.NewGaslessSwapRouter(addr, transactor)
	if err != nil {
		t.Fatal(err)
	}
...
func setupLiquidity(t *testing.T, owner *TestAccountType, contracts contractsForGasless, node *cn.CN) {
	...
	addLiquidityTx, err := routerContract.AddLiquidityETH(optsForAddLiquidity, testTokenAddr,
		initialLiquidity, common.Big1, common.Big1, owner.Addr, big.NewInt(deadline))
	if err != nil {
		t.Fatal(err)
	}
